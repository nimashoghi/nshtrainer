...
from ..callbacks.checkpoint import CheckpointCallbackConfig
from ..loggers import (
    CSVLoggerConfig,
    LoggerConfig,
...
from ..profilers import ProfilerConfig
...
class LoggingConfig(CallbackConfigBase):
    ...
    def create_loggers(self, trainer_config: "TrainerConfig"):
        ...
        Args:
            trainer_config (TrainerConfig): The root configuration for the trainer.
...
class RunConfig(C.Config):
    ...
    def append_to_name_(self, *parts: str, sep: str = "-"):
        """
        Append parts to the name of the run.

        Args:
            *parts (str): The parts to append to the name.
            sep (str, optional): The separator to use between parts. Defaults to "-".
...
class TrainerConfig(C.Config):
    ...
    def metric_or_primary_metric(self, metric: MetricConfig | None):
        if metric is not None:
            return metric

        return self.primary_metric

    def dir_or_default_subdir(self, dir: Path | None, subdirectory: str):
        if dir is not None:
            return dir
        return self.directory.resolve_subdirectory(self.run.id, subdirectory)
